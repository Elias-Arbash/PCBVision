...
def resize_hyperspectral_images(HSI_list, Mask_list, resolution=640):
    """
    Resizes a list of hyperspectral images and their corresponding segmentation masks to the specified resolution.
...
def resize_segmentation_masks(mask, new_shape):
    """
    Resize a list of segmentation masks while preserving class values to the specified new shape (row x col).

    Parameters:
        masks (list of numpy arrays): A list of segmentation masks with values [0, 1, 2, 3].
        new_shape (tuple): A tuple representing the new shape (row, col) to which the masks should be resized.
